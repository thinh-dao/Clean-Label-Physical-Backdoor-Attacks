...
class NAD():
    ...
    def __init__(self, args, defense_loader, source_testloader, target_class, valid_loader, teacher_epochs=10, erase_epochs=20):
        self.args = args
    ...
    def train_teacher(self):
        ...
        teacher = get_model(self.args.net[0]).cuda()
        teacher.load_state_dict(torch.load("post_attack/backdoored_model_9-5_sunglasses.pth"))
        t_model_path = os.path.join(self.folder_path, 'NAD_T.pt')
        ...
        for epoch in range(0, self.teacher_epochs):
            ...
            #     test(teacher, test_loader=self.valid_loader, poison_test=True, poison_transform=self.poison_transform, num_classes=self.num_classes, source_classes=self.source_classes, all_to_all=('all_to_all' in self.args.dataset))

            self.train_step(self.defense_loader, teacher, optimizer, criterion, epoch+1)

            # evaluate on testing set
            # acc_clean, acc_bad = test(teacher, test_loader=self.valid_loader, poison_test=True, poison_transform=self.poison_transform, num_classes=self.num_classes, source_classes=self.source_classes, all_to_all=('all_to_all' in self.args.dataset))
        ...
        acc_clean, acc_bad = self.test(teacher)
        t_model_path = os.path.join(self.folder_path, 'NAD_T.pt')
        self.save_checkpoint(teacher.module.state_dict(), True, t_model_path)
    ...
    def test(self, model):
        ...
        with torch.no_grad():
            for data, _ in self.source_testloader:
                data = data.cuda()
                output = model(data)
                pred = output.argmax(dim=1)
    ...
    def train_erase(self):
        ...
        # Load models
        print('----------- Network Initialization --------------')        
        teacher = get_model(self.args.net[0])
        t_model_path = os.path.join(self.folder_path, 'NAD_T.pt')
        ...
        student = get_model(self.args.net[0]).cuda()
        student.load_state_dict(torch.load("post_attack/backdoored_model_9-5_sunglasses.pth"))
        ...
        for epoch in range(0, self.erase_epochs):
            ...
            #     test(student, test_loader=self.valid_loader, poison_test=True, poison_transform=self.poison_transform, num_classes=self.num_classes, source_classes=self.source_classes, all_to_all=('all_to_all' in self.args.dataset))

            self.train_step_erase(self.defense_loader, nets, optimizer, criterions, self.betas, epoch+1)

            # evaluate on testing set
            # acc_clean, acc_bad = self.test_erase(nets, criterions, self.betas, epoch+1)
            # acc_clean, acc_bad = acc_clean[0], acc_bad[0]
            # acc_clean, acc_bad = test(student, test_loader=self.valid_loader, poison_test=True, poison_transform=self.poison_transform, num_classes=self.num_classes, source_classes=self.source_classes, all_to_all=('all_to_all' in self.args.dataset))
    ...
    def test_erase(self, nets, criterions, betas, epoch):
        ...
        for idx, (img, target) in enumerate(self.valid_loader, start=1):
            img = img.cuda()
            target = target.cuda()
